...
Temporary Credentials / Token
   │
   ├── AWS: STS credentials (Access Key / Secret / Token)
...
👉 Key:
- Credentials issued in Control Plane
- Used in Data Plane
...
Executes inside VM
   │
   ▼
Returns output/logs to service
...
Ingestion Service
   │
   ├── AWS: CloudWatch (logs/metrics ingestion)
...
Stored in monitoring backend
   │
   ▼
Queried via Control Plane (dashboards / alerts)